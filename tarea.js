...
function tienenMismaLongitud(str1, str2) {
  ...
  const agregarAmigo = (usuario, nuevoAmigo) => {
    ...
    const user = {
      ...usuario,
      amigos: [...usuario.amigos, nuevoAmigo]
    }
    return user
  }
  ...
  const sumarLikesDeUsuario = usuario => {
    ...
    // Devuelve la suma
    // Tu código:
    const suma = usuario.posts.map(post => post.likes).reduce((acum, item) => acum + item)
    return suma
  }
...
const sumarArray = (numeros, cb) =>{
    ...
    const sum = numeros.reduce((prev, actual) => prev + actual)
    cb(sum)
}
